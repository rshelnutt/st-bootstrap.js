...
// Populate Category Products Data
...
// Set Position of Brand Text/Logo
...
function setViewportClass() {
    ...
    var searchFormUI2 = $('.ui-widgetTN2');
    var searchFormContainer = $(searchFormUI).parent();
    var searchFormContainer2 = $(searchFormUI2).parent();
    var searchForm = $('#SearchForm');
    var searchInput = $('#SearchText');
    var searchFormMobile = $('#SearchFormMobile');
    var searchTextMobileDummy = $('#SearchTextDummy');
    var prodImgLink = $('.COMProdContainer .MainProdBox a');
    ...
    if ($(window).width() < 992) { // If viewport is Small Desktop (eBook, small laptop, etc)
        ...
        // Add Mobile Classes To Body
            $('body').addClass('mobile').removeClass('full mobile-sm');
        ...
        // Set Brand Positioning & Styles
            ...
            $('.nav-item.navbar-brand').addClass('pl-3');
            $('.nav-item.navbar-brand').removeClass('pr-3');
            $('.nav-item.navbar-brand').removeClass('mr-0');
        ...
        if ($(window).width() < 768) { // If viewport is Tablet
            ...
            $('.nav-item.navbar-brand').addClass('pr-3');
        ...
        $('.nav-item.navbar-brand').removeClass('pl-3');
        $('.nav-item.navbar-brand').addClass('mr-3');
        ...
        $('body').addClass('full').removeClass('mobile mobile-sm mobile-xs');
        $('#mainMenuNav').find('.dropdown-toggle').each(function() {
            $(this).attr({
                "data-toggle" : "",
                "data-hover" : "dropdown"
...
/*    var winH = ($(window).height() - $('#customFiltersToggle').height())-55;
    if ($('body').hasClass('mobile')) {
        $('#customFiltersWrap').css('width','0');
        $('#customFiltersToggle').css('top',winH);
        ...
        $('#customFiltersWrap').removeClass('collapse show');
        $('#customFilters').removeClass('card card-body p-3');
    } else if ($('body').hasClass('full')) {
        $('#customFilters').show(0);
        $('#customFiltersWrap').css('width','inherit');
        $('#body').removeClass('mobilemodalOpen');
        $('#blackout').css({
            'opacity':'0',
            'pointer-events':'none'
        });
        $('#customFiltersWrap').addClass('collapse');
        $('#customFilters').addClass('card card-body p-3');
    }*/
}

function customFiltersToggle() {
/*    if( $('.mobile #customFilters').is(':visible') ) {
        $('html').removeAttr('style');
        $('html').css({'overflow-x':'hidden'});
        $('.mobile #customFiltersWrap').animate({ 'width': '0px' }, 200);
        $('#body').removeClass('mobilemodalOpen');
        $('#blackout').css({
            'opacity':'0',
            'pointer-events':'none'
        ...
        $('.mobile #customFilters').addClass('filtersOpen');
        $('html').removeAttr('style');
        $('html').css({'overflow':'hidden'});
        $('#body').addClass('mobilemodalOpen');
        $('.mobile #customFiltersWrap').animate({ 'width': '60vw' }, 200);
        $('#blackout').fadeTo( 200, 1 ).css({
            'opacity':'100%',
            'pointer-events':'all'
        });
    }
    $('.mobile #customFilters').toggle( "fade", 200 );*/
}

function checkScroll() {
    var x = document.getElementById("body").scrollHeight;
...
//Set Fullwidth Heights
function setFullwidthHeight(){
    $('.fullwidth').each(function(){
        var topRow = $("[id^=row-]");
        var fullHeight = $(this).outerHeight();

        $(this).closest(topRow).addClass="row-fullwidth";
        $(this).closest(topRow).css('height',fullHeight);
    });
}
...
$(document).ready( function() {
    ...
    // Set custom branding text or image on page
        if (stappConfig.cfgBrandImg === "1") {
            var x = $.trim(stappConfig.cfgBrand);
            $('.navbar-brand,.nav-side-logo').html(x); //load config file
        } else {
            var x = $.trim(stappConfig.cfgShopName);
            $('.navbar-brand,.nav-side-logo').text(x); //load config file
        }
    ...
    // Construct blackout behind on custom filters
    //     $('.mobile #customFilters').fadeOut(0);
    //     $('#customFiltersWrap').before('<div id="blackout"></div>');

    // Toggle filters close when clicking blackout
...
/*        hero = $(this);
        ...
        var heroDataType = hero.attr('data-type');
        var heroImg = hero.children('img');
        ...
        if (hero.hasClass('fill fullscreen')) {
            hero.parent().parent().addClass('hero-stage fill fullscreen');
        } else if (hero.hasClass('fill')) {
            hero.parent().parent().addClass('hero-stage fill');
        }
        else if (hero.hasClass('fullscreen')) {
            hero.parent().parent().addClass('hero-stage fullscreen');
        } else {
            hero.parent().parent().addClass('hero-stage');
        }*/
...
$('.slider.slider-media').each(function(slider) {
    ...
    slider.find('li').each(function() {
        ...
        if($(this).is(':contains("youtube.com")')) {
            ...
            var vidStartLink = "";
            var vidPlaylist = $(this).attr('data-video-playlist');
            var vidPlaylistLink = "";
